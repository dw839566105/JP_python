...
# read file with .h5
if len(sys.argv)<2:
    filepath = r'./output_alpha' # target dir, can be changed!
    fig_path = './figure_alpha'
    log_path = '.'
else:
    filepath = sys.argv[1]
...
if not os.path.exists(fig_path): # prevent exist
    os.mkdir(fig_path)
print(np.max(recon_total[:,0]))
print(np.min(recon_total[:,0]))
# fig_0 E and tau_d
...
# plot 2d
# EE = np.hstack((np.arange(0.2,0.8,0.1),np.arange(1.1,2.3,0.1)))
EE = np.arange(np.min(recon_total[:,0]), np.max(recon_total[:,0]) \
    - (np.max(recon_total[:,0])-np.min(recon_total[:,0]))/10, \
  (np.max(recon_total[:,0])-np.min(recon_total[:,0]))/10) # energy
...
plt.xlabel('Radius: [m]', size=20)
plt.ylabel('Energy: [MeV]', size=20)
plt.title(r'Energy resolution /$\sqrt{E}$', size=20)
plt.subplot(122)
