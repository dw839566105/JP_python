...
def con_m(args):
    ...
    cons = ({'type': 'ineq', 'fun': lambda x: x[0]*(100-x[0])})
...
def recon(fid, fout, miter=None):
    ...
    for event in TruthChain:
        ...
        result = minimize(Recon_e(), x0, method='SLSQP',constraints=cons_e)
        ...
        cons_m =con_m(args)
        result = minimize(Recon_m(), t0, method='SLSQP',constraints=cons_m)
